...
contract DepositAdapterTest is TestSetup {
    ...
    function setUp() public {
        ...
        stETHmainnet = IERC20Upgradeable(
        ...
        address depositAdapterProxy = address(
            new UUPSProxy(depositAdapterImpl, "")
        );
    ...
    function test_DepositWeETH() public {
        ...
        uint256 weEthAmount = depositAdapterInstance.depositETHForWeETH{
            value: 1 ether
        }(address(0));
        assertApproxEqAbs(
            weEthInstance.balanceOf(address(alice)),
        ...
        assertApproxEqAbs(
            weEthInstance.balanceOf(address(alice)),
            weEthInstance.getWeETHByeETH(1000 ether) + balanceBeforeDeposit,
            3
        );
    ...
    function test_DepositWETH() public {
        ...
        // valid wETH deposit
        uint256 liquidityPoolBalanceBeforeDeposit = address(
        ...
        assertEq(
            ...
            liquidityPoolBalanceBeforeDeposit + 1 ether
        );
        assertApproxEqAbs(
            weEthInstance.balanceOf(address(alice)),
            weEthInstance.getWeETHByeETH(1 ether),
    ...
    function test_DepositStETH() public {
        ...
        ILiquifier.PermitInput memory liquifierPermitInput = ILiquifier
            .PermitInput({
        ...
        depositAdapterInstance.depositStETHForWeETHWithPermit(
            1 ether,
            address(0),
            liquifierPermitInput
        );

        // empty request
        permitInput = createPermitInput(
            2,
            address(depositAdapterInstance),
            0,
            stEth.nonces(alice),
        ...
        liquifierPermitInput = ILiquifier.PermitInput({
            value: permitInput.value,
            deadline: permitInput.deadline,
            v: permitInput.v,
            r: permitInput.r,
        ...
        // valid input
        uint256 protocolStETHBeforeDeposit = stEth.balanceOf(
            address(liquifierInstance)
        );
        ...
        assertApproxEqAbs(
            stEth.balanceOf(address(alice)),
            stEthBalanceBeforeDeposit - 1 ether,
            3
        );
        assertApproxEqAbs(
            weEthInstance.balanceOf(address(alice)),
        ...
        assertApproxEqAbs(
            stEth.balanceOf(address(liquifierInstance)),
            protocolStETHBeforeDeposit + 1 ether,
            3
        );
        ...
        protocolStETHBeforeDeposit = stEth.balanceOf(
            address(liquifierInstance)
        );
        permitInput = createPermitInput(
            2,
        ...
        assertApproxEqAbs(
            stEth.balanceOf(address(liquifierInstance)),
            protocolStETHBeforeDeposit + 5000 ether,
            3
        );
    }
    ...
    function test_DepositWstETH() public {
        ...
        uint256 protocolSeETHBeforeDeposit = stEth.balanceOf(
            address(liquifierInstance)
        );
        ILiquidityPool.PermitInput memory permitInput = createPermitInput(
            2,
            address(depositAdapterInstance),
            wstETHAmount,
            wstETHmainnet.nonces(alice),
            2 ** 256 - 1,
            wstETHmainnet.DOMAIN_SEPARATOR()
        );
        ILiquifier.PermitInput memory liquifierPermitInput = ILiquifier
            .PermitInput({
                value: permitInput.value,
        ...
        depositAdapterInstance.depositWstETHForWeETHWithPermit(
            wstETHAmount,
            bob,
            liquifierPermitInput
        );
        ...
        assertApproxEqAbs(
            weEthInstance.balanceOf(address(alice)),
            weEthInstance.getWeETHByeETH(5 ether),
            3
        );
        assertApproxEqAbs(
            stEth.balanceOf(address(liquifierInstance)),
            protocolSeETHBeforeDeposit + 5 ether,
            3
        );
