    ...
    IMembershipManager
{
    ...
    event FundsMigrated(
        ...
        uint256 _amount,
        uint256 _eapPoints,
        uint40 _loyaltyPoints,
        uint40 _tierPoints
    );
    event NftUpdated(
        uint256 _tokenId,
        uint128 _amount,
        uint128 _amountSacrificedForBoostingPoints,
        uint40 _loyaltyPoints,
        uint40 _tierPoints,
        uint8 _tier,
        uint32 _prevTopUpTimestamp,
        uint96 _share
    );
    event NftUnwrappedForEEth(
        address indexed _user,
        uint256 indexed _tokenId,
        uint256 _amountOfEEth,
        uint40 _loyaltyPoints,
        uint256 _feeAmount
    );
    ...
    function initialize(
        ...
        treasury = _treasury;
        DEPRECATED_protocolRevenueManager = _protocolRevenueManager;

        // Set default values
        fanBoostThreshold = uint16(0.3 ether / 0.001 ether); // 0.3 ether converted to the contract's unit
        burnFeeWaiverPeriodInDays = 183;
    ...
    function wrapEthForEap(
        ...
        if (
            ...
            msg.value > _snapshotEthAmount * 2 ||
            msg.value != _amount + _amountForPoints
        ) revert InvalidEAPRollover();

        membershipNFT.processDepositFromEapUser(
            msg.sender,
        ...
        uint40 tierPoints = membershipNFT.computeTierPointsForEap(
            _eapDepositBlockNumber
        );

        liquidityPool.deposit{value: msg.value}(msg.sender, address(0));

        uint256 tokenId = _mintMembershipNFT(
            msg.sender,
            msg.value - _amountForPoints,
            _amountForPoints,
            loyaltyPoints,
            tierPoints
        );
    ...
    function wrapEth(
        ...
        if (
            depositPerNFT / 1 gwei < minDepositGwei ||
            msg.value != ethNeededPerNFT
    ...
    /// @param _amount amount of ETH to earn staking rewards.
    /// @param _amountForPoints amount of ETH to boost earnings of {loyalty, tier} points
    function topUpDepositWithEth(
        uint256 _tokenId,
        uint128 _amount,
        uint128 _amountForPoints
    ) public payable whenNotPaused {
        _requireTokenOwner(_tokenId);

        claim(_tokenId);

        uint256 additionalDeposit = _topUpDeposit(
            _tokenId,
            _amount,
            _amountForPoints
        );
    ...
    function requestWithdraw(
        ...
        claim(_tokenId);
        if (!membershipNFT.isWithdrawable(_tokenId, _amount))
            revert ExceededMaxWithdrawal();

        uint256 prevAmount = ethAmountForVaultShare(
            tokenData[_tokenId].tier,
            tokenData[_tokenId].vaultShare
        );
    ...
    function rebase(int128 _accruedRewards) external firewallProtected {
        ...
        uint256 ethRewardsPerEEthShareAfterRebase = liquidityPool
            .amountForShare(1 ether);
        ...
        if (address(this).balance >= thresholdAmount) {
            ...
            ethRewardsPerEEthShareAfterRebase +=
                (1 ether * thresholdAmount) /
                etherFanEEthShares;
        }

        _distributeStakingRewardsV0(
            ethRewardsPerEEthShareBeforeRebase,
            ethRewardsPerEEthShareAfterRebase
        );
        _distributeStakingRewardsV1(
            ethRewardsPerEEthShareBeforeRebase,
            ethRewardsPerEEthShareAfterRebase
        );
    }
    ...
    function _distributeStakingRewardsV0(
        uint256 _ethRewardsPerEEthShareBeforeRebase,
        uint256 _ethRewardsPerEEthShareAfterRebase
    ) internal {
        uint96[] memory globalIndex = globalIndexLibrary.calculateGlobalIndex(
            address(this),
            address(liquidityPool),
            _ethRewardsPerEEthShareBeforeRebase,
            _ethRewardsPerEEthShareAfterRebase
        );
        for (uint256 i = 0; i < tierDeposits.length; i++) {
            tierDeposits[i].shares = uint128(
                liquidityPool.sharesForAmount(tierDeposits[i].amounts)
            );
            tierData[i].rewardsGlobalIndex = globalIndex[i];
        }
    }

    function _distributeStakingRewardsV1(
        uint256 _ethRewardsPerEEthShareBeforeRebase,
        uint256 _ethRewardsPerEEthShareAfterRebase
    ) internal {
        uint128[] memory vaultTotalPooledEEthShares = globalIndexLibrary
            .calculateVaultEEthShares(
                address(this),
                address(liquidityPool),
                _ethRewardsPerEEthShareBeforeRebase,
                _ethRewardsPerEEthShareAfterRebase
    ...
    function _deposit(
        ...
        uint256 eEthShare = liquidityPool.sharesForAmount(
            _amount + _amountForPoints
        );
    ...
    function _topUpDeposit(
        ...
        if (!canTopUp(_tokenId, additionalDeposit, _amount, _amountForPoints))
            revert InvalidDeposit();

        TokenData storage token = tokenData[_tokenId];
        uint256 totalDeposit = ethAmountForVaultShare(
            token.tier,
            token.vaultShare
        );
        uint256 maxDepositWithoutPenalty = (totalDeposit *
            maxDepositTopUpPercent) / 100;
        ...
        if (additionalDeposit > maxDepositWithoutPenalty) {
            uint256 dilutedPoints = (totalDeposit * token.baseTierPoints) /
    ...
    function _wrapEth(
        ...
        liquidityPool.deposit{value: _amount + _amountForPoints}(
            msg.sender,
            _referral
        );
        uint256 tokenId = _mintMembershipNFT(
    ...
    function _withdrawAndBurn(
        ...
        uint256 feeAmount = hasMetBurnFeeWaiverPeriod(_tokenId)
            ? 0
            : uint256(burnFee) * 0.001 ether;
    ...
    function _withdraw(uint256 _tokenId, uint256 _amount) internal {
        if (membershipNFT.valueOf(_tokenId) < _amount)
    ...
    // V0
    function _incrementTokenDeposit(
        uint256 _tokenId,
        uint256 _amount
    ) internal {
        ...
        tokenDeposits[_tokenId] = TokenDeposit(newAmount, newShare);
    }

    function _decrementTokenDeposit(
        uint256 _tokenId,
        uint256 _amount
    ) internal {
        TokenDeposit memory deposit = tokenDeposits[_tokenId];
    ...
    function _incrementTierDeposit(uint256 _tier, uint256 _amount) internal {
        ...
        uint128 newAmount = deposit.amounts + uint128(_amount);
        uint128 newShare = uint128(liquidityPool.sharesForAmount(newAmount));
        tierDeposits[_tier] = TierDeposit(newAmount, newShare);
    }
    ...
    // V1
    function _incrementTokenVaultShareV1(
        uint256 _tokenId,
        uint256 _share
    ) internal {
    ...
    function _decrementTokenVaultShareV1(
        uint256 _tokenId,
        uint256 _share
    ) internal {
        tokenData[_tokenId].vaultShare -= uint96(_share);
    ...
    function _incrementTierVaultV1(
        uint8 _tier,
        uint256 _eEthShare,
        uint256 _vaultShare
    ) internal {
    ...
    function _decrementTierVaultV1(
        uint8 _tier,
        uint256 _eEthShare,
        uint256 _vaultShare
    ) internal {
    ...
    function eEthShareForVaultShare(
        uint8 _tier,
        uint256 _vaultShare
        ...
        if (tierVaults[_tier].totalVaultShares == 0) {
            ...
            amount =
                (_vaultShare * tierVaults[_tier].totalPooledEEthShares) /
                tierVaults[_tier].totalVaultShares;
        }
        return amount;
    }

    function vaultShareForEEthShare(
        uint8 _tier,
        uint256 _eEthShare
    ) public view returns (uint256) {
        uint256 vaultShare;
        if (tierVaults[_tier].totalPooledEEthShares == 0) {
            ...
            vaultShare =
                (_eEthShare * tierVaults[_tier].totalVaultShares) /
                tierVaults[_tier].totalPooledEEthShares;
        }
        return vaultShare;
    }

    function ethAmountForVaultShare(
        uint8 _tier,
        uint256 _vaultShare
    ) public view returns (uint256) {
    ...
    function vaultShareForEthAmount(
        uint8 _tier,
        uint256 _ethAmount
    ) public view returns (uint256) {
    ...
    function hasMetBurnFeeWaiverPeriod(
        uint256 _tokenId
    ...
    function _updateAllTimeHighDepositOf(uint256 _tokenId) internal {
        allTimeHighDepositAmount[_tokenId] = membershipNFT.allTimeHighDepositOf(
            _tokenId
        );
    }
    ...
    function _requireTokenOwner(uint256 _tokenId) internal view {
        if (membershipNFT.balanceOfUser(msg.sender, _tokenId) != 1)
    ...
    function _feeAmountSanityCheck(uint256 _feeAmount) internal pure {
        if (
            _feeAmount % 0.001 ether != 0 ||
            _feeAmount / 0.001 ether > type(uint16).max
    ...
    function _applyUnwrapPenalty(
        ...
        uint40 degradeTierPenalty = curTierPoints -
            tierData[prevTier].requiredTierPoints;

        // point deduction if scaled proportional to withdrawal amount
        uint256 ratio = (10000 * _withdrawalAmount) / _prevAmount;
        uint40 scaledTierPointsPenalty = uint40(
            (ratio * curTierPoints) / 10000
        );

        uint40 penalty = uint40(
            _max(degradeTierPenalty, scaledTierPointsPenalty)
        );
    ...
    function _emitNftUpdateEvent(uint256 _tokenId) internal {
        ...
        emit NftUpdated(
            ...
            amount,
            0,
            token.baseLoyaltyPoints,
            token.baseTierPoints,
            token.tier,
            token.prevTopUpTimestamp,
            token.vaultShare
        );
    }
    ...
    function tierForPoints(uint40 _tierPoints) public view returns (uint8) {
        ...
        while (
            tierId < tierData.length &&
            _tierPoints >= tierData[tierId].requiredTierPoints
        ) {
            tierId++;
        }
    ...
    function canTopUp(
        ...
        if (block.timestamp - uint256(prevTopUpTimestamp) < topUpCooltimePeriod)
            return false;
